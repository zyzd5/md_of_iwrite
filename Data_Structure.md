...
#### 二叉排序树(二叉搜索树, 二叉排序树, BST(binary search tree)树)
* 查找, 插入, 删除 时间复杂度均为 log(n)

* 左节点 < 根节点 < 右节点

* 左子树和右子树又各是一棵二叉排序树

[数据结构合集 - 二叉搜索树(二叉排序树)(二叉查找树)](https://www.bilibili.com/video/BV1uK421x7JJ/?share_source=copy_web&vd_source=6fcaeb589ebc683e202dd787a3b58143)
#### 平衡二叉树(平衡二叉搜索树, AVL树)
* 平衡二叉树：(左子树高度-右子树高度)的绝对值 <= 1

* 每个节点都有一个`平衡因子`, (等于 `左子树高度` - `右子树高度`)

* 查找, 插入, 构建, 删除和二叉搜索树一致, 只是失衡时需要调整

* `左旋` 和 `右旋`: 很难解释, 等用到了再看吧
#### 顺序存储
